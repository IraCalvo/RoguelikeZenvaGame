...
public class Generation : MonoBehaviour
{
    ...
    void CheckRoom(int x, int y, int remaining, Vector2 generalDirection, bool firstRoom = false)
    {
        ...
        if(north || firstRoom)
            CheckRoom(x, y + 1, firstRoom ? maxRemaning : remaining - 1, firstRoom ? Vector2.up : generalDirection);

        if(south || firstRoom)
            CheckRoom(x, y - 1, firstRoom ? maxRemaning : remaining - 1, firstRoom ? Vector2.down : generalDirection);

        if(east || firstRoom)
            CheckRoom(x + 1, y, firstRoom ? maxRemaning : remaining - 1, firstRoom ? Vector2.right : generalDirection);

        if(west || firstRoom)
            CheckRoom(x - 1, y, firstRoom ? maxRemaning : remaining - 1, firstRoom ? Vector2.left : generalDirection);
    }
    ...
    void InstantiateRooms()
    {
        ...
        //this creates a loop so that it creates the map until it's all filled
